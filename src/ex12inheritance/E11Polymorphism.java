...
다형성(Polymorphism)
	...
	4.이런경우를 이질화(Heterogeneous:헤테로지니어스)라고한다.
...
class Parent{
	...
	void parentMetod() {
		System.out.println("부모의메소드:parentMetod()");
	}
}
...
class Child extends Parent {
	...
	@Override
	void parentMetod() {
		System.out.println("자식에서 Overriding한 메소드 parentMetod()");
		super.parentMetod();
	}

	void parentMetod(int childMember) {
...
public class E11Polymorphism {
	...
	public static void main(String[] args) {
		...
		homeChild.parentMetod(1000);//오버로딩한 메서드
		homeChild.parentMetod();//오버라이딩한 메서드
		//동질화의 경우 객체 전체를 접근할 수 있다.
		
		
		/*
		이질화 : 부모타입의 변수로 자식 객체를 참조하는것.
			이경우 자식에서 새롭게 정의한 멤버는 접근할 수 없다.
		...
		상송한다. 따라서 모든 객체는 Object의 참조변수로 참조가능하다.
		또한 Object클래스에 정의된 모든 메서드를 별도의 선언없이
		사용할 수 있따.
		 */
		Object object = new Child();
		((Parent)object).parentMetod();
	}
}