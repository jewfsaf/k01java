...
오버라이딩 : 메서드명, 매개변수, 반환형까지 모두 동일한 메서드를
	...
	이 경우 하위클래스에서 정의한 메서드가 상위클래스이 메서드를
...
class C extends B {
	/*
	reidMethod()는 오버라이딩 처리된 메서드로 상위클래스에
...
public class E07RideAndLoad01 {
	...
	public static void main(String[] args) {
		...
		/*
		참조변수에 상관없이 항상 최상위에 오버라이딩된 메서드가
		...
		//ref2는 B타입이므로 접근볌위가 B까지로 제한된다.
		ref2.loadMethod();
		ref2.loadMethod(1);
		//ref2.loadMethod(1.1); -> C클래스 영역은 접근불가
		
		//ref1는 A타입이므로 접근볌위가 A까지로 제한된다.
		ref1.loadMethod();
		...
		/*
		참조변수 refId1 이 C객체를 참조하고 있으므로 마치 
		rdfId1이 C타입이라고 착각할수 있으나, 처음부터 refId1은
		A타입의 참조변수이다.
		자식으로 부모를 참조하는것은 불가능하므로 아래 2개의 
		문장은 에러가 발생한다.
		 */
		A refId1 = new C();  //가능
